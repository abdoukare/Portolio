...
const expertise = [
  { 
    ...
    skills: ["Python", "PostgreSQL", "RESTAPI", "Golang", "Node.js", "MongoDB", "Redis"]
  },
  { 
    title: "DevOps & Cloud", 
    ...
    skills: ["GithubActions","Docker", "CI/CD", "Linux", "Git"]
  },
];
    ...
    <section ref={sectionRef} id="expertise" className="relative min-h-screen w-full bg-[#030305] overflow-hidden flex items-center justify-center py-20">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div 
          ...
          style={{
            background: `
              radial-gradient(ellipse at ${50 + mousePos.x}% ${50 + mousePos.y}%, rgba(59, 130, 246, 0.3) 0%, transparent 50%),
              radial-gradient(ellipse at ${30 - mousePos.x}% ${70 - mousePos.y}%, rgba(96, 165, 250, 0.2) 0%, transparent 50%),
              radial-gradient(ellipse at ${70 + mousePos.x}% ${30 + mousePos.y}%, rgba(96, 165, 250, 0.2) 0%, transparent 50%)
            `,
      ...
      {/* Floating orbs */}
      <div className="absolute top-20 left-[10%] w-2 h-2 rounded-full bg-blue-500 float-slow opacity-60 pointer-events-none" />
      <div className="absolute top-40 right-[15%] w-3 h-3 rounded-full bg-sky-500 float-fast opacity-50 pointer-events-none" />
      <div className="absolute bottom-32 left-[20%] w-2 h-2 rounded-full bg-blue-500 float-slow opacity-40 pointer-events-none" />
      <div className="absolute top-1/2 right-[8%] w-4 h-4 rounded-full bg-blue-400/30 float-fast pointer-events-none" />
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8" style={{ perspective: "1200px" }}>
        ...
        <div className="section-header text-center mb-12 sm:mb-16">
          <h3 className="mt-2 sm:mt-3 text-3xl sm:text-4xl lg:text-5xl font-bold text-white">
            Areas of <span className="bg-gradient-to-r from-blue-400 to-blue-400 bg-clip-text text-transparent">Expertise</span>
          </h3>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
          {expertise.map((item, index) => (
            ...
            >
              <div className="relative h-full p-5 sm:p-6 lg:p-8 rounded-2xl bg-zinc-900/50 border border-zinc-800/50 backdrop-blur-sm overflow-hidden transition-all duration-500 hover:border-blue-500/30 hover:bg-zinc-900/80 hover:-translate-y-1">
                ...
                <div className="relative flex flex-col sm:flex-row gap-4 sm:gap-5">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col">
                    <h4 className="text-lg lg:text-xl font-bold text-white group-hover:text-blue-400 transition-colors">
                      {item.title}
                    </h4>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {item.skills.map((skill, i) => (
                        <span 
                          key={i}
                          className="skill-tag px-2.5 py-1 text-xs rounded-full bg-zinc-800/80 text-zinc-300 border border-zinc-700/50 hover:border-blue-500/50 hover:text-white transition-all cursor-default"
                        >
                          {skill}
                        </span>
                      ))}
